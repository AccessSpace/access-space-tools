...
class syntax_plugin_linkmap_linklist extends DokuWiki_Syntax_Plugin {
    ...
    function getInfo() {
        return array(
          'author' => 'Martyn Eggleton for access-space.org',
                ...
                'name'   => 'Linkmap Plugin (linklist component)',
    ...
    function connectTo($mode) {
        $this->Lexer->addSpecialPattern('\{\{linklist>.*?\}\}',$mode,'plugin_linkmap_linklist');
    }

    function handle($match, $state, $pos, &$handler) {
        global $ID;
        
        $match = substr($match, 11, -2); // strip {{linkmap> from start and }} from end
    ...
    function render($mode, &$renderer, $data)
    {
      ...
      if ($mode == 'xhtml')
      {
        ...
        $aRendered = $oMap->render_list($sSearch);
