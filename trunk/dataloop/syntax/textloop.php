...
class syntax_plugin_dataloop_textloop extends DokuWiki_Syntax_Plugin {
     ...
     */
    function syntax_plugin_dataloop_textloop(){
     ...
     */
    function getInfo(){
        return $this->loophelper->getInfo('Text Loop');
    }
    ...
    function connectTo($mode) {
			 $this->Lexer->addEntryPattern('<datatextloop.*?>(?=.*?</datatextloop>)',$mode,'plugin_dataloop_textloop');
       //$this->Lexer->addPattern('----+ *datatextloop(?: [ a-zA-Z0-9_]*)?-+\n.*?\n----+',$mode,'plugin_dataloop_textloop');
    }
		
		function postConnect() { 
			$this->Lexer->addExitPattern('</datatextloop>','plugin_dataloop_textloop'); 
		}
    ...
    function handle($match, $state, $pos, &$handler){
        ...
        case DOKU_LEXER_ENTER :
					...
					$class = str_replace('datatextloop','',$class);
					$class = trim($class,'- ');
    ...
    function render($format, &$renderer, $alldata) {
        ...
        $renderer->info['cache'] = false;
				...
				$sDataLoopSeperator = "£$%DATATEXTLOOP£$%";
        ...
        switch ($state) {
              ...
              case DOKU_LEXER_EXIT :
									...
									$sCurrentText = $sBlockText;
										...
										foreach($row as $num => $col){
											...
											$xValue =  $this->dthlp->_formatData($cols[$num],$col,$types[$num],$renderer);
												...
												$sCurrentText = str_ireplace('@@'.$cols[$num].'@@', $xValue, $sCurrentText);
											}
								...
								if($data['limit']){
										$renderer->doc .= '<div class="dataplugin_textloop limit">';
